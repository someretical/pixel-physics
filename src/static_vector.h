...
#define TL_STATIC_VECTOR_VERSION_NAMESPACE_CONCAT_HELPER(id1, id2, id3) v_##id1##_##id2##_##id3
...
#define TL_STATIC_VECTOR_VERSION_NAMESPACE                                                                             \
    TL_STATIC_VECTOR_VERSION_NAMESPACE_CONCAT(                                                                         \
        TL_STATIC_VECTOR_VERSION_MAJOR,                                                                                \
...
template <class Exception> [[noreturn]] inline constexpr void ThrowOrAbort(const char *expression_str) {
...
inline constexpr void ThrowIfOrAbort(const bool expression_eval, const char *expression_str) {
    if (expression_eval) {
...
// checked.
template <class T> using EnableIfLegacyInputIterator = std::enable_if_t<std::input_iterator<T>>;
...
public:
    ...
    template <class InputIt, class = internal::EnableIfLegacyInputIterator<InputIt>>
    ...
    constexpr StaticVector(std::initializer_list<T> init) : StaticVector(init.begin(), init.end()) {}
    ...
    constexpr auto insert(const const_iterator pos, const size_type count, const T &value) -> iterator {
    ...
    template <class InputIt, class = internal::EnableIfLegacyInputIterator<InputIt>>
    constexpr auto insert(const const_iterator pos, InputIt first, InputIt last) -> iterator {
        const size_type count = std::distance(first, last);
    ...
    constexpr auto insert(const const_iterator pos, std::initializer_list<T> ilist) -> iterator {
        return insert(pos, ilist.begin(), ilist.end());
    }

    // Inserts a new element into the container directly before pos.
    template <class... Args> constexpr auto emplace(const const_iterator pos, Args &&...args) -> iterator {
        TL_STATIC_VECTOR_THROW_IF(std::length_error, size() + 1 > max_size());
    ...
    template <class... Args> constexpr auto emplace_back(Args &&...args) -> reference {
        TL_STATIC_VECTOR_THROW_IF(std::length_error, size() + 1 > max_size());
...
private:
    ...
    constexpr auto GetElementPointer(const size_type index) const noexcept -> const T * {
        return &data()[index];
    }
    ...
    auto MoveElementsBack(const const_iterator pos, const size_type count) -> iterator {
...
constexpr auto operator==(const StaticVector<T, N> &lhs, const StaticVector<T, M> &rhs) -> bool {
...
template <class T, std::size_t N, std::size_t M>
constexpr auto operator<(const StaticVector<T, N> &lhs, const StaticVector<T, M> &rhs) {
...
template <class T, std::size_t N> constexpr void swap(StaticVector<T, N> &lhs, StaticVector<T, N> &rhs) noexcept {
    lhs.swap(rhs);
}
...
constexpr auto erase(StaticVector<T, N> &c, const U &value) -> typename StaticVector<T, N>::size_type {
...
constexpr auto erase_if(StaticVector<T, N> &c, Pred pred) -> typename StaticVector<T, N>::size_type {
